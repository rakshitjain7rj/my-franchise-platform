...
export default function HeaderSearch({
  ...
  const updatePanelPos = useCallback(() => {
    ...
    // Prefer aligning to the right edge of the field so a narrow desktop field
    // still gets a readable panel without overflowing the viewport.
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // Only clear loading if this request is still the latest (a newer
        // keystroke already set loading true / will set false itself).
        if (
    ...
    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
      // Abort in-flight work immediately when the query changes — do not wait
      // for the next debounced fetch to start (that left a stale-apply window).
      abortRef.current?.abort();
    };
  }, [value]);

  // Cleanup abort on unmount (belt-and-suspenders with effect cleanup above)
  useEffect(() => {
    return () => {
      abortRef.current?.abort();
    };
  ...
  // Fixed position so the panel is not clipped by sticky header / mobile drawer overflow
  useLayoutEffect(() => {
    if (!showPanel) {
      setPanelPos(null);
      return;
    ...
    // capture scroll from nested drawers too
    window.addEventListener("scroll", onReposition, true);
    return () => {
      window.removeEventListener("resize", onReposition);
      window.removeEventListener("scroll", onReposition, true);
    };
  }, [showPanel, updatePanelPos, hits.length, loading]);

  // Click outside closes panel (input root + fixed panel)
  useEffect(() => {
    if (!open) return;
  ...
  const maxIndex = hits.length; // last selectable = "see all"
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    // activeIndex === footerIndex ("See all") or none → catalogue (intentional)
    goCatalogue(value);
  };
    ...
    <div ref={rootRef} className={`relative flex items-center ${className}`}>
      ...
      >
        <div className="relative flex w-full items-center">
          ...
          <input
            ...
            className="h-10 w-full min-w-0 rounded-full border border-purple-100 bg-purple-50/40 pl-11 pr-9 text-sm font-medium text-purple-950 placeholder:text-purple-400/80 transition-all focus:border-purple-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-200/60 sm:w-44 md:w-52 lg:w-64 [&::-webkit-search-cancel-button]:hidden"
          />
          {value && (
            <button
              type="button"
      ...
      {showPanel && panelPos && (
        ...
        >
          ...
          {/* SR announcement of result count (visual UI already shows hits) */}
